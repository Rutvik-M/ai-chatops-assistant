...
# --- Page config ---
st.set_page_config(page_title="AI ChatOps Assistant", page_icon="🤖", layout="wide")
...
if st.session_state.get("authentication_status") == True:
    st.success("SUCCESS: You are logged in.")
elif st.session_state.get("authentication_status") == False:
    st.error("Username/password failed")
else:
...
if st.session_state.get("authentication_status") == False:
    ...
    st.title(f"Welcome, *{name}* (Role: {user_role})")
    st.header("AI ChatOps Assistant 🤖")
    ...
    with st.sidebar:
        ...
        if st.button("🔧 Open API Tools", key="api_tools_btn"):
        ...
        if user_role in ["admin", "hr"]:  # Only admin/HR can re-index
            if st.button("🔄 Re-index Knowledge Base", key="reindex_btn"):
                with st.spinner("🔄 Re-indexing knowledge base... This may take 1-2 minutes."):
                    try:
                        import subprocess
                        import sys
                        ...
                        if result.returncode == 0:
                            ...
                            st.info("📚 New files are now searchable in the chat")
                        ...
                        st.error("❌ Re-indexing timed out (took >5 minutes)")
                    except Exception as e:
                        st.error(f"❌ Error: {e}")
            
            st.caption("💡 Click after adding new files to `knowledge_base/` folder")
            ...
            if os.path.exists("db/chroma.sqlite3"):
                try:
                    ...
                    st.caption(f"🕒 Last indexed: {last_time.strftime('%Y-%m-%d %H:%M:%S')}")
                except Exception:
                    pass  # Silently fail if can't read timestamp
        else:
            st.info("🔒 Knowledge base management available to admin/HR only")
    ...
    if agent_key not in st.session_state:
        ...
        if MOCK_AGENT:
            ...
            with st.spinner(f"Loading AI agent for '{username}'..."):
                try:
    ...
    if "messages" not in st.session_state:
        st.session_state.messages = [
            {"role": "assistant", "content": "Hello! How can I help you with our company knowledge base today?", "message_id": "initial"}
        ]
    ...
    if st.session_state.get("show_api_tools", False):
        with st.expander("🔗 API Integrations & Tools", expanded=False):
            ...
            try:
                ...
                with api_tab1:
                    ...
                    with col1:
                        if st.button("📋 Get My Employee Info", key="get_emp_info_btn"):
                    ...
                    with col2:
                        ...
                        if st.button("✅ Use PTO Days", key="use_pto_btn"):
                ...
                with api_tab2:
                    ...
                    if user_role in ["admin", "it_support"]:
                        ...
                        with support_tab1:
                            ...
                            # Get all tickets
                            if st.button("🔄 Refresh Tickets", key="refresh_all_tickets"):
                                st.rerun()
                ...
                with api_tab3:
                    st.markdown("#### 📊 API Integration Statistics")
